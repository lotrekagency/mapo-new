...
| `useCrud<T>()` — full CRUD repository        | `list`, `detail`, `create`, `update`, `partialUpdate`, `delete`, `options`, `updateOrCreate`, `updateOrder`                                                                                                                                                             |
...
| `useFormFromSchema(schema, options)`                                    | JSON Schema → `FieldDescriptor[]` (Pydantic / DRF), `if/then/else` → `visible()`, `readOnly` → `readonly`, `format: textarea`, `exclude`, `overrides`, cycle-safe `$ref` |
